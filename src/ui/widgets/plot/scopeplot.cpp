...
string ScopePlot::add_curve(ScopeCurveData *curve_data)
{
	...
	// TODO
	connect(curve_data, &ScopeCurveData::reset_curve,
		this, [&]() { curve->set_painted_points(0); replot(); });
	// TODO
	connect(curve_data, &ScopeCurveData::update_curve,
		this, [&]() { update_intervals(); update_curves(); });
...
bool ScopePlot::add_curve(Curve *curve)
{
...
void ScopePlot::remove_curve(Curve *curve)
{
	// Delete markers
...
int ScopePlot::init_x_axis(BaseCurveData *curve_data, int x_axis_id)
{
	assert(curve_data);
...
int ScopePlot::init_y_axis(BaseCurveData *curve_data, int y_axis_id)
{
	assert(curve_data);
...
void ScopePlot::add_marker(sv::ui::widgets::plot::Curve *curve)
{
...
// TODO
void ScopePlot::update_curve(/*string curve_id*/)
{
	for (const auto &curve : curve_map_) {
		curve.second->set_painted_points(0);
	}
	...
	/*
	auto curve = curve_map_[curve_id];
...
bool ScopePlot::update_x_interval(Curve *curve)
{
...
bool ScopePlot::update_y_interval(const Curve *curve)
{
...
Curve *ScopePlot::get_curve_from_plot_curve(const QwtPlotCurve *plot_curve) const
{
	for (const auto &curve : curve_map_) {
