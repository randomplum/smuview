...
void ScopeChannel::start_new_frame(double timestamp, uint64_t samplerate)
{
	if (samplerate == 0) {
		qWarning() << "ScopeChannel::start_new_frame(): samplerate is 0!";
		return;
	}
	...
	shared_ptr<sigrok::Analog> sr_analog)
{
	...
	*/
	...
	static_pointer_cast<data::AnalogScopeSignal>(actual_signal_)->push_samples(
		...
		size_of_float_ /*sr_analog->unitsize()*/, digits, decimal_places);
}
