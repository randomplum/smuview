...
void AnalogChannel::start_new_frame(double timestamp, uint64_t samplerate)
{
	(void)timestamp;
	(void)samplerate;
}

void AnalogChannel::push_interleaved_samples(const float *data,
	const size_t sample_count, const size_t channel_stride,
	const double timestamp, const uint64_t samplerate,
	const uint64_t sample_interval, shared_ptr<sigrok::Analog> sr_analog)
{
	...
	static_pointer_cast<data::AnalogTimeSignal>(actual_signal_)->push_samples(
		deint_data.get(), sample_count, timestamp, time_stride,
